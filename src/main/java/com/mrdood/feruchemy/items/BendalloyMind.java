...
public class BendalloyMind extends ModItemSuper{
	...
	public ItemStack onItemRightClick(ItemStack stack, World world, EntityPlayer player){
		if(!world.isRemote){
			...
			if(player.isSneaking() && player.getFoodStats().getFoodLevel() > 0 && player.getDisplayName().equals(nbt.getString("mindOwner"))){
				player.getFoodStats().setFoodLevel(player.getFoodStats().getFoodLevel()-1);
				nbt.setInteger("stored", nbt.getInteger("stored") + 1);
				//System.out.println(player.experienceLevel);
			}else if(!player.isSneaking() && nbt.getInteger("stored") > 0 && player.getDisplayName().equals(nbt.getString("mindOwner")) && player.getFoodStats().needFood()){
				player.getFoodStats().setFoodLevel(player.getFoodStats().getFoodLevel()+1);
				nbt.setInteger("stored", nbt.getInteger("stored") - 1);
				//System.out.println(player.experienceLevel);
	...
	public void addInformation(ItemStack stack,EntityPlayer player,List lores,boolean b){
		if(stack.hasTagCompound() && stack.getTagCompound().hasKey("mindOwner")){
			...
			lores.add("Nutrition Stored: "+(stack.getTagCompound().getInteger("stored")));
		}
	}

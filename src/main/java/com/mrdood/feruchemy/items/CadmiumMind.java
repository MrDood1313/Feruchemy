...
public class CadmiumMind extends ModItemSuper{
	...
	public void onUpdate(ItemStack stack, World world, Entity player, int pInt, boolean b){
	    ...
	    if(((EntityPlayer) player).getDisplayName().equals(nbt.getString("mindOwner"))){
		    ...
		    if(nbt.getInteger("stored") > 0 && nbt.getBoolean("isTapping")){
				...
				((EntityLivingBase) player).addPotionEffect(new PotionEffect(Potion.waterBreathing.getId(),20,0));
				if(!world.isAirBlock((int)player.posX, (int)player.posY,(int) player.posZ)){
					((EntityLivingBase) player).addPotionEffect(new PotionEffect(Potion.resistance.getId(),20,3));
				}
		    }
	...
	public void addInformation(ItemStack stack,EntityPlayer player,List lores,boolean b){
		if(stack.hasTagCompound() && stack.getTagCompound().hasKey("mindOwner")){
			...
			lores.add("Breath Stored: "+(stack.getTagCompound().getInteger("stored")/10.0)+"%");
